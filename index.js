...
app.use(session({secret:' namaste',
...
app.post('/login', async (req, res) => {
 ...
 app.get('/updateprofile',async (req,res)=>{
  ...
  console.log("loginmail:" +  req.session.loginMail)
  let email=req.session.loginMail
  ...
  try {
   ...
   let response= `<!DOCTYPE html>
      ...
      <body>
          ...
          <form  class="center-form" action="http://localhost:3000/updateprofile" method="POST" id="signup">
            <H4>REGISTRATION</H4>
            <p><b><font size=2px color=black></font>     <input type="text" id="fname" name="fname"   value="${user.fname}" required></b></p>
 ...
 app.post('/updateprofile',async(req,res)=>{
  ...
  console.log(req.body.porg)
  console.log(req.body.exp)
  console.log(req.body.skill)
  let email=req.session.loginMail
  try {
    const user = await User.findOneAndUpdate( {email:req.session.loginMail},{
...
app.get('/changepassword',(req,res)=>{
  ...
  console.log("password chng loginmail:" +  req.session.loginMail)
 ...
 console.log(req.body)
  ...
  try {
    ...
    if (passwordMatch) {
      ...
      User.findOneAndUpdate({email:req.session.loginMail},{hashedPassword:newhashedPassword})
      .then(docs=>{
